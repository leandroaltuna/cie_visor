...
class Tank_auth
{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					//Added
					$roles = $this->ci->roles->get_roles_by_user($user->id);
					$ubigeo = $this->ci->roles->get_ubigeo($user->id)->ubigeo;
	...
	function logout()
	{
		...
		// See http:///forums/viewreply/662369/ as the reason for the next line
